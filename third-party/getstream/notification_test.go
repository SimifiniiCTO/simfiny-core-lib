...
func TestClient_SendFollowRequestNotification(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				params: &FollowRequestActivity{
					SourceActor:           utils.GenerateRandomString(10),
					ActionName:            utils.GenerateRandomString(10),
					FollowRequestRecordID: utils.GenerateRandomString(5),
					Time:                  time.Time{},
					ActivityForeignID:     utils.GenerateRandomString(5),
				},
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.f.SendFollowRequestNotification(tt.args.ctx, tt.args.notificationFeedID, tt.args.params); (err != nil) != tt.wantErr {
