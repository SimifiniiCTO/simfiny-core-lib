...
func TestClient_FollowFeed(t *testing.T) {
	sourceFlatFeed := generateFlatFeed(t, utils.GenerateRandomString(5))
	targetFlatFeed := generateFlatFeed(t, utils.GenerateRandomString(5))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.f.FollowFeed(tt.args.ctx, tt.args.sourceFeedID, tt.args.targetFeedID, tt.args.activityCopyLimit); (err != nil) != tt.wantErr {
