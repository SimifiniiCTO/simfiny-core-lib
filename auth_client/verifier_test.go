...
func TestIDTokenVerifier(t *testing.T) {
	...
	// build a verifier
	jwks := &mockJwkProvider{keyMap: map[string]jose.JSONWebKey{}}
	jwks.keyMap[defaultJWK.KeyID] = jose.JSONWebKey{Key: defaultKey.Public(), KeyID: "defaultKey"}
	issuer := "https://lib.example.com"
	...
	t.Run("URL-equivalent issuer", func(t *testing.T) {
		...
		testClaims.Issuer = "https://lib.example.com/"
	...
	t.Run("alg=hmac attack", func(t *testing.T) {
		...
		// it to verify.
		jwkJSON, err := jwks.keyMap[defaultJWK.KeyID].MarshalJSON()
		require.NoError(t, err)
	...
	t.Run("wrong issuer", func(t *testing.T) {
		testClaims := defaultClaims
		testClaims.Issuer = "https://lib.elsewhere.com"
		token, err := jwt.Signed(defaultSigner).Claims(testClaims).CompactSerialize()
		require.NoError(t, err)
